...
export default function LandslideSafety() {
  ...
  const checkpoints = [
    { id: 0, type: "intro", title: "Introduction to Landslide Safety", content: "Landslides can occur suddenly on slopes. Learn to recognize warning signs and protect yourself!", icon: "⛰️" },
    { id: 1, type: "video", title: "Watch: Landslide Awareness", videoUrl: "https://www.youtube.com/embed/VcgoZlpn1Y4", icon: "📹" },
    { id: 2, type: "content", title: "Before a Landslide", content: ["Know if you live in landslide-prone area", "Watch for changes in landscape", "Plant ground cover on slopes", "Build retaining walls if needed", "Have evacuation plan ready"], icon: "⚠️" },
    { id: 3, type: "quiz", title: "Checkpoint Quiz: Preparation", questions: [
      { question: "What increases landslide risk?", options: ["Heavy rainfall", "Dry weather", "Cold temperature", "Wind"], correct: 0 },
      { question: "What is a warning sign of landslide?", options: ["Birds chirping", "Cracks in ground", "Sunny day", "Clear sky"], correct: 1 },
      { question: "Where should you avoid building?", options: ["Flat land", "Steep slopes", "Valley", "Plains"], correct: 1 },
      { question: "What helps prevent landslides?", options: ["Removing vegetation", "Planting ground cover", "Adding water", "Digging"], correct: 1 },
      { question: "Should you monitor slope changes?", options: ["No, unnecessary", "Yes, regularly", "Only in summer", "Never"], correct: 1 }
    ], icon: "✅" },
    { id: 4, type: "content", title: "During a Landslide", content: ["Move away from the path immediately", "Go to higher ground perpendicular to flow", "Stay alert for flooding", "Curl into tight ball if caught", "Protect your head"], icon: "🚨" },
    { id: 5, type: "quiz", title: "Checkpoint Quiz: Response", questions: [
      { question: "If you see landslide coming, what to do?", options: ["Run downhill", "Move perpendicular to flow", "Stand still", "Run toward it"], correct: 1 },
      { question: "Can landslides cause floods?", options: ["No, never", "Yes, by blocking rivers", "Only in winter", "Impossible"], correct: 1 },
      { question: "What sound indicates landslide?", options: ["Birds singing", "Rumbling sound", "Silence", "Music"], correct: 1 },
      { question: "If caught, what should you do?", options: ["Stand up", "Curl into ball, protect head", "Run", "Scream"], correct: 1 },
      { question: "Should you stay indoors during landslide?", options: ["Yes, always safe", "No, evacuate if possible", "Only at night", "Yes, in basement"], correct: 1 }
    ], icon: "✅" },
    { id: 6, type: "content", title: "After a Landslide", content: ["Stay away from slide area", "Watch for additional slides", "Check for injured people", "Report broken utilities", "Listen to authorities"], icon: "🏠" },
    { id: 7, type: "final-quiz", title: "Final Assessment", questions: [
      { question: "What triggers most landslides?", options: ["Earthquakes only", "Heavy rain and earthquakes", "Wind", "Snow"], correct: 1 },
      { question: "Can landslides happen without warning?", options: ["No, always warned", "Yes, can be sudden", "Only at night", "Never sudden"], correct: 1 },
      { question: "When is it safe to return?", options: ["Immediately", "When authorities clear it", "Next day", "After rain stops"], correct: 1 },
      { question: "What should you report after landslide?", options: ["Nothing", "Broken utilities and hazards", "Only injuries", "Weather"], correct: 1 },
      { question: "Can small slides lead to bigger ones?", options: ["No, never", "Yes, watch for more", "Only in mountains", "Impossible"], correct: 1 }
    ], icon: "🏆" }
  ];

  useEffect(() => {
    const user = JSON.parse(localStorage.getItem("user") || "{}");
    const progressKey = user.email ? `${user.email}_landslideProgress` : "landslideProgress";
  ...
  const saveProgress = (checkpoint, completed, newScore) => {
    localStorage.setItem("landslideProgress", JSON.stringify({ currentCheckpoint: checkpoint, completedCheckpoints: completed, score: newScore }));
  };
  ...
  const generateCertificate = () => {
    ...
    doc.setFontSize(20);
    doc.setFont(undefined, 'bold');
    doc.text('Landslide Safety Course', 148.5, 135, { align: 'center' });
    
    doc.setFontSize(14);
    doc.setFont(undefined, 'normal');
    doc.text(`Score: ${score.toFixed(0)} points | Date: ${date}`, 148.5, 155, { align: 'center' });
    
    doc.save(`${userName}_LandslideSafety_Certificate.pdf`);
  };
  ...
  const oldGenerateCertificate = () => {
    ...
    alert(`Certificate Generated!\n\nThis certifies that ${userName} has successfully completed the Landslide Safety course on ${date}.\n\nScore: ${score.toFixed(0)} points\nCompletion: 100%`);
  };
      ...
      <div className="gamified-learning-page">
        ...
        <div className="learning-header">
          <div className="module-title-section">
            ...
            <h1>Landslide Safety Mastery</h1>
          </div>
